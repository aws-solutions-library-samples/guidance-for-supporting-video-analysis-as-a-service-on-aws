...
use crate::hybrid_streaming_service::kvs_callbacks::fragment_ack::{
    OfflineKVSFrameAckManager, KVS_FRAGMENT_OFFLINE_ACK_MANAGER,
};
use glib::ObjectType;
...
#[cfg(feature = "sd-card-catchup")]
unsafe extern "C" fn fragment_ack_callback_offline_c_abi(
    _sink: *mut ffi::GstElement,
    fragment_ack: *mut __FragmentAck,
    _: glib::Pointer,
) {
    if fragment_ack.is_null() {
        warn!("KVS offline client passed in a null value for Fragment Ack!");
        return;
    }
    // Extern C methods should be unsafe as C can pass in null pointers which can conflict with safe rust.
    // To take full advantage of Rust compiler core logic is held within an inline function to minimize the
    // unsafe sections of code to accessing C pointers only.
    // NEVER USE ANY PARAMETER PASSED IN OTHER THAN FRAGMENT_ACK!
    // Inject KVS OFFLINE manager so callback sends messages on the correct channel.
    fragment_ack_callback_offline(fragment_ack, KVS_FRAGMENT_OFFLINE_ACK_MANAGER.clone());
}
...
#[cfg(feature = "sd-card-catchup")]
#[inline]
fn fragment_ack_callback_offline(
    fragment_ack: *mut __FragmentAck,
    fragment_manager: Arc<Mutex<OfflineKVSFrameAckManager>>,
) {
    match unsafe { (*fragment_ack).ackType } {
        FRAGMENT_ACK_TYPE_FRAGMENT_ACK_TYPE_UNDEFINED => {
            warn!("KVS Fragment was undefined, time-code = {}", unsafe {
                (*fragment_ack).timestamp
            });
        }
        FRAGMENT_ACK_TYPE_FRAGMENT_ACK_TYPE_ERROR => {
            warn!("KVS Fragment returned an error, time-code = {}", unsafe {
                (*fragment_ack).timestamp
            });
            warn!(
                "KVS Fragment offline returned an error, the error type code is = {:?}",
                unsafe { (*fragment_ack).result }
            );
        }
        // Generate timeline information
        FRAGMENT_ACK_TYPE_FRAGMENT_ACK_TYPE_PERSISTED => {
            ...
            let ack_fragment_timestamp = unsafe { (*fragment_ack).timestamp };
            match fragment_ack_manager_lock
                .try_send_persisted_fragment_time_code(ack_fragment_timestamp)
            {
                Ok(_) => debug!("KVS Fragment offline published persisted time-code to kvs_fragment_ack, time-code = {} channel.", unsafe { (*fragment_ack).timestamp }),
                Err(e) => error!("Message failed to publish to callback channel : {:?}", e),
            };
        }
        _ => {
            debug!("KVS Fragment unknown, time-code = {}", unsafe { (*fragment_ack).timestamp });
            debug!("KVS Fragment offline ack type is = {:?}", unsafe { (*fragment_ack).ackType });
            debug!(
                "KVS Fragment offline returned an error, the error type code is = {:?}",
                unsafe { (*fragment_ack).result }
            );
...
/// This will be called by the KVS sync plugin
#[cfg(feature = "sd-card-catchup")]
pub(crate) fn setup_kvs_fragment_ack_callback_offline(kvssink: &Element) {
    let f = Box::new(fragment_ack_callback_offline_c_abi);
    unsafe {
        glib::signal::connect_raw(
            kvssink.as_ptr() as *mut _,
            b"fragment-ack\0".as_ptr() as *const _,
            Some(mem::transmute::<_, unsafe extern "C" fn()>(
                fragment_ack_callback_offline_c_abi as *const (),
            )),
            Box::into_raw(f),
        )
    };
