...
/// This file holds Rust structs of the WS-Discovery APIs that are relevant
...
pub mod endpoint_reference {
    ...
    #[yaserde(prefix = "wsa", namespace = "wsa: http://schemas.xmlsoap.org/ws/2004/08/addressing")]
...
pub mod probe_matches {
    use crate::wsdl_rs::ws_discovery::endpoint_reference::EndpointReference;
    ...
    impl ProbeMatch {
        ...
        fn split_string_to_urls(s: &str) -> Vec<Url> {
            s.split_whitespace().filter_map(|addr| Url::parse(addr).ok()).collect()
        }
    }
    ...
    fn probe_match() {
        ...
        assert!(de.find_in_scopes("onvif://www.onvif.org/type/video_encoder").is_some());
        assert!(de.find_in_scopes("onvif://www.onvif.org/type/video_analytics").is_none());
        assert_eq!(
            de.x_addrs(),
            vec![
